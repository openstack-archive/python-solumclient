...
log_fixture = [
    {
        'assembly_uuid': 'a1',
        'location': '/var/log/solum/worker/unittest-z1.log',
        'strategy': 'local',
        'strategy_info': '{}',
    },
]
...
class AssemblyManagerTest(base.TestCase):
    ...
    def assert_logs_object(self, logs_obj, assembly_id):
        self.assertIsNotNone(logs_obj)
        for log in logs_obj:
            self.assertEqual(log.assembly_uuid, assembly_id)
            self.assertIsNotNone(log.location)
            self.assertIsNotNone(log.strategy)
            self.assertIsNotNone(log.strategy_info)
    ...
    def test_list_logs(self):
        ...
        self.assert_logs_object(logs_obj, 'a1')
